...
const FIREBASE_API_KEY = "AIzaSyA0seDdHj7OhNf6hx2-AZIDn1qhQpbFLTk";

const DATABASE_URL =

  "https://messenger-e50bf-default-rtdb.europe-west1.firebasedatabase.app";

const ALLOWED_ORIGIN = "https://aliyahclacken-hue.github.io";
    ...
    try {
      ...
      if (!recipientUid || !chatId || !messageText) {
        ...
        return jsonResponse(
          ...
          { error: "Notification information is missing." },
      ...
      if (recipientUid === sender.localId) {
        ...
        return jsonResponse(
          ...
          { error: "You cannot notify your own account." },
      ...
      const members = await readFirebasePath(
      ...
      if (
        ...
        return jsonResponse(
          ...
          { error: "You are not both members of this chat." },
      ...
      const tokenRecords = await readFirebasePath(

        `notificationTokens/${encodeURIComponent(recipientUid)}`,

        accessToken

      );

      if (!tokenRecords || typeof tokenRecords !== "object") {
        ...
        return jsonResponse(

          {
            ...
            message: "The other person has not enabled notifications."
      ...
      const deviceTokens = Object.values(tokenRecords)
...
async function readFirebasePath(path, accessToken) {
        ...
        message: {
          ...
          webpush: {
            ...
            fcm_options: {
              ...
              link: "https://aliyahclacken-hue.github.io/messenger-/"
...
async function getGoogleAccessToken(env) {
  ...
  if (!response.ok || !data.access_token) {
    ...
    throw new Error(
      ...
      `Google sign-in error: ${JSON.stringify(data)}`
